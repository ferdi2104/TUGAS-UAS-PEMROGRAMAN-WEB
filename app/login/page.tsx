  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-surface-container border border-primary/20 rounded-2xl p-8 shadow-[0_0_40px_rgba(255,45,120,0.1)]">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-primary text-on-primary font-bold font-label uppercase tracking-wider py-3 rounded-lg hover:shadow-[0_0_20px_rgba(255,45,120,0.5)] hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              {loading ? (
        ...
        <p className="text-center text-on-surface-variant/40 text-xs font-label mt-6 uppercase tracking-widest">
          NEON-ACTION Cinema © 2024
        </p>
      </div>
    </main>
  );
  ...
  return (
    <Suspense fallback={
      <main className="min-h-screen flex items-center justify-center">
        <div className="text-primary font-label animate-pulse">Loading...</div>
      </main>
    }>
      <LoginForm />
    </Suspense>
