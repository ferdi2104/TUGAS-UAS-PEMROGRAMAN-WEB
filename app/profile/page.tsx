    ...
    <main className="pt-24 pb-12 min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-12">
          <div className="w-28 h-28 rounded-full border-2 border-secondary overflow-hidden bg-surface-variant flex items-center justify-center flex-shrink-0">
            <span className="material-symbols-outlined text-5xl text-on-surface-variant">person</span>
          </div>
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-3xl font-black font-headline text-on-surface mb-2">{username || 'Student Agent'}</h1>
            <p className="text-on-surface-variant font-body mb-4">{email || 'Belum ada email'}</p>
            <div className="flex flex-wrap justify-center md:justify-start gap-3">
              <Link href="/pencarian">
                <button className="bg-primary text-on-primary font-label font-bold px-5 py-2 text-sm transition-all hover:shadow-[0_0_15px_rgba(255,45,120,0.5)] active:scale-95">
                  BROWSE MOVIES
                </button>
              </Link>
              <Link href="/pencarian">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {[
            ...
            <div key={i} className="bg-surface-container rounded-xl border border-outline/10 p-5 text-center">
              ...
              <p className="text-xs font-label text-on-surface-variant uppercase tracking-widest mt-1">{stat.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-surface-container rounded-xl border border-outline/10 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  className="w-full bg-surface-container-highest border border-outline/30 rounded-lg px-4 py-2.5 text-on-surface placeholder:text-on-surface-variant/40 focus:border-secondary focus:ring-1 focus:ring-secondary/50 transition-all"
          ...
          {/* Student Pass */}
          <div className="bg-surface-container rounded-xl border border-secondary/20 p-6 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4 text-secondary">
                ...
                <span className="font-label text-sm font-bold uppercase tracking-widest">Student Pass</span>
              </div>
              <h3 className="text-2xl font-black font-headline text-on-surface mb-2">FREE TIER</h3>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-body ${feature.active ? 'text-on-surface' : 'text-on-surface-variant/60'}`}>{feature.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {watchHistory.map((item, i) => (
                <Link key={i} href={`/detail?id=${item.id}`} className="group">
                  ...
                  <p className="text-sm font-label text-on-surface mt-2 truncate">{item.title}</p>
                </Link>
              ))}
            </div>
          ) : (
            <div className="text-center py-10">
              <span className="material-symbols-outlined text-4xl text-on-surface-variant/40 mb-3">movie</span>
              <p className="text-on-surface-variant font-body">Belum ada riwayat nonton.</p>
              <Link href="/pencarian">
                <button className="mt-4 bg-primary/20 text-primary border border-primary/40 px-6 py-2 font-label text-sm font-bold hover:bg-primary hover:text-on-primary transition-all">
                  EXPLORE MOVIES
                </button>
              </Link>
            </div>
