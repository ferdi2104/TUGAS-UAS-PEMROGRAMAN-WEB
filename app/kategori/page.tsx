...
export default function KategoriPage() {
  ...
  return (
    <main className="pt-24 pb-20 relative overflow-hidden min-h-screen">
      ...
      <section className="container mx-auto px-4 sm:px-6 relative z-10 mb-10 sm:mb-16">
        <div className="max-w-3xl">
          <h1 className="text-3xl sm:text-5xl md:text-7xl font-headline font-extrabold text-on-surface tracking-tighter leading-none mb-4">
            CHOOSE YOUR <span className="text-primary neon-text-pink">BATTLEGROUND</span>
          </h1>
          <p className="text-on-surface-variant font-body text-sm sm:text-lg leading-relaxed max-w-xl">
      ...
      <section className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6">
          {display.map((cat, i: number) => {
            const isLarge = i === 0;
            const isWide = i === 3;
            const heights = isLarge ? 'h-[250px] sm:h-[300px] md:h-auto md:col-span-2 md:row-span-2' : isWide ? 'h-[200px] sm:h-[250px] md:col-span-2' : 'h-[220px] sm:h-[300px]';
            ...
            return (
              <Link key={cat.id || i} href={`/pencarian?genre=${genreMap[cat.name] || cat.name || 'Action'}`} className={`${heights} category-card group cursor-pointer relative overflow-hidden rounded-xl`}>
                ...
                <div className={`absolute bottom-0 ${isLarge || isWide ? 'p-4 sm:p-8' : 'p-3 sm:p-6'} w-full`}>
                  {cat.tag && (
                    <span className={`font-label ${cat.color === 'secondary' ? 'text-secondary' : cat.color === 'tertiary' ? 'text-tertiary' : 'text-primary'} text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] block mb-1`}>
                      {cat.tag || cat.color}
                    </span>
                  )}
                  <h3 className={`${isLarge ? 'text-2xl sm:text-4xl' : 'text-lg sm:text-2xl'} font-headline font-bold text-on-surface tracking-tight`}>{cat.name}</h3>
                  {cat.description && <p className="text-on-surface-variant text-xs sm:text-sm mt-2 max-w-sm">{cat.description}</p>}
                </div>
              </Link>
            );
          })}
      ...
      <section className="container mx-auto px-4 sm:px-6 py-10 sm:py-20 relative z-10">
        <div className="bg-surface-container/50 border border-secondary/20 rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row justify-around items-center gap-6 sm:gap-8 backdrop-blur-sm">
          <div className="text-center">
            <span className="block text-3xl sm:text-4xl font-headline font-black text-secondary neon-text-cyan mb-2">1,240+</span>
            <span className="font-label uppercase text-[10px] sm:text-xs tracking-widest text-on-surface-variant">Live Missions</span>
          </div>
          <div className="w-px h-12 bg-secondary/20 hidden md:block"></div>
          <div className="text-center">
            <span className="block text-3xl sm:text-4xl font-headline font-black text-primary neon-text-pink mb-2">48</span>
            <span className="font-label uppercase text-[10px] sm:text-xs tracking-widest text-on-surface-variant">Sub-Genres</span>
          </div>
          <div className="w-px h-12 bg-secondary/20 hidden md:block"></div>
          <div className="text-center">
            <span className="block text-3xl sm:text-4xl font-headline font-black text-tertiary mb-2">24/7</span>
            <span className="font-label uppercase text-[10px] sm:text-xs tracking-widest text-on-surface-variant">Active Stream</span>
          </div>
        </div>
      </section>
